...
function moment(input?: any, format?: any, localeOrStrict?: any, fourthArg?: any): Moment {
  ...
  if (input === undefined) {
    ...
    return new Moment({ _dClone: false, _d: new Date(nowFn()), _i: input });
  }
...
function createFromArray(arr: any[], isUTC?: boolean): Moment {
  if (arr.length === 0) return new Moment({ _dClone: false, _d: new Date(nowFn()), _i: arr });
  let hasNull = false;
  for (const v of arr) {
...
function createFromObject(obj: Record<string, any>): Moment {
  ...
  if (isObjectEmpty(parsed)) return new Moment({ _dClone: false, _d: new Date(nowFn()), _i: obj });
  const now = new Date(nowFn());
...
Object.defineProperty(Moment.prototype, "clone", {
  value: Moment.prototype.clone,
  writable: true,
  configurable: true,
  enumerable: true,
});

...
const { esmCodeCoverage } = require("./plugins/esmCodeCoverage.js");
const { compareHooks } = require("./plugins/compareHooks.js");
const { sourceCodeCoverage } = require("./plugins/sourceCodeCoverage.js");
const { functionHooks } = require("./plugins/functionHooks.js");
...
const { hookManager: loaderHookManager } = require("@jazzer.js/hooking");
const { setSeed } = require("./plugins/helpers.js");
...
export const load = async function load(url, context, nextLoad) {
    ...
    // result.source can be string, ArrayBuffer, SharedArrayBuffer, or
    // Uint8Array.  Plain .toString() on the buffer types produces garbage
    // ("[object ArrayBuffer]" or comma-separated bytes), so decode explicitly.
    const code = typeof result.source === "string"
        ? result.source
        : Buffer.from(result.source instanceof Uint8Array
            ? result.source
            : new Uint8Array(result.source)).toString("utf-8");
...
// ── Instrumentation ──────────────────────────────────────────────
function instrumentModule(code, filename) {
    drainHookUpdates();
    const fuzzerCoverage = esmCodeCoverage();
    const plugins = [fuzzerCoverage.plugin, compareHooks];
    ...
    if (transformed.map) {
        ...
        preambleLines.push(`__jazzer_registerSourceMap(${JSON.stringify(filename)}, ${JSON.stringify(shifted)});`);
    }
 ...
 */
function drainHookUpdates() {
    if (!loaderPort)
        return;
    let msg;
    while ((msg = receiveMessageOnPort(loaderPort))) {
        const hooks = msg.message.hooks;
        for (const h of hooks) {
            const stub = loaderHookManager.registerHook(h.type, h.target, h.pkg, h.async, noop);
            ...
            if (actualId !== h.id) {
                throw new Error(`ESM hook ID mismatch: expected ${h.id}, got ${actualId} ` +

...
## パフォーマンス（bench-datefns2, warm median）
...
### 7. diff months — !float 高速パス復元（`src/moment_fixed.ts`）
- `diff()` から削除されていた `!float` 算術専用パスを復元
- Date 生成ゼロで `anchorMs` の `new Date(y,m,d...).getTime()` 3回を除去

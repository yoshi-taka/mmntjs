...
import { getRelTimeThreshold, getRelTimeRounding } from "./moment";
...
export class Duration {
  ...
  private _parseObject(obj: DurationInput): void {
    if (hasOwnProp(obj, "from") || hasOwnProp(obj, "to")) {
      const { momentFromAnything, Moment } = require("./moment");
  ...
  get(unit: string): number {
    ...
    switch (key) {
      ...
      case "weeks":
        return absFloor(this._data.days / 7);
      case "months":
  ...
  seconds(n?: number): number | Duration {
    if (!this._isValid) return NaN as any;
    if (n !== undefined) {
      const diff = n - this._data.seconds;
  ...
  minutes(n?: number): number | Duration {
    if (!this._isValid) return NaN as any;
    if (n !== undefined) {
      const diff = n - this._data.minutes;
  ...
  months(n?: number): number | Duration {
    if (!this._isValid) return NaN as any;
    if (n !== undefined) {
      const diff = n - this._data.months;
      this._months += diff;
      this._bubble();
      return this;
    }
  ...
  weeks(n?: number): number | Duration {
    if (!this._isValid) return NaN as any;
    if (n !== undefined) {
      const diff = n - absFloor(this._data.days / 7);
      this._days += diff * 7;
      this._bubble();
      return this;
    }
  ...
  humanize(
    ...
    const totalDays = this._days;
    const totalMonths = this._months;
    const monthDayBase = Math.round(monthsToDays(totalMonths));
    const absDays = Math.abs(totalDays + monthDayBase);
    const absMs = Math.abs(totalMs);

    const seconds = roundFn(absDays * 86400 + absMs / 1000);
    const minutes = roundFn(absDays * 1440 + absMs / 60000);
    const hours = roundFn(absDays * 24 + absMs / 3600000);
    const days = roundFn(absDays + absMs / 86400000);
    const weeks = roundFn(absDays / 7 + absMs / 604800000);

    const monthVal = Math.abs(totalMonths + daysToMonths(totalDays + totalMs / 86400000));
    const months = roundFn(monthVal);
    const years = roundFn(monthVal / 12);

...
let mutTotal = 0, mutKilled = 0, mutSurvived = 0, mutSkipped = 0
...
makeMutations([
  ...
  },
  {
    ...
    file: 'src/moment-class.ts',
    patterns: [
      [/this\._t \+= rounded \* 86400000;/g, 'this._t -= rounded * 86400000;'],
    ],
    inputs: fc.tuple(fc.date({ noInvalidDate: true }), nonZeroInt(-100, 100)),
    testFn: (input: unknown) => {
      const [date, n] = input as [unknown, unknown];
      return (mutatedMoment as unknown as Record<string, (x: unknown) => Moment>).utc(date).add(n as number, 'days').format('YYYY-MM-DD') === (originalMoment as unknown as Record<string, (x: unknown) => Moment>).utc(date as Date).add(n as number, 'days').format('YYYY-MM-DD')
    },
  },
  {
  ...
  },
  {
    ...
    testFn: (input: unknown) => {
      ...
      return mutatedMoment(d).isBefore(new Date(d.getTime() + 86400000), 'day') === originalMoment(d).isBefore(new Date(d.getTime() + 86400000), 'day')
    },
  },
  {
  ...
  // === Phase 2: parse.ts ===
  {
    ...
    file: 'src/parse.ts',
    patterns: [
      [/dayOfYear >= 1 && dayOfYear <= 366/g, 'dayOfYear >= 0 && dayOfYear <= 366'],
    ],
  ...
  // === Phase 2: duration.ts ===
  {
    ...
    patterns: [
      [/days -= absCeil\(monthsToDays\(monthsFromDays\)\);/g, 'days += absCeil(monthsToDays(monthsFromDays));'],
    ],
  ...
  },
  {
    ...
    file: 'src/duration.ts',
    patterns: [
      [/daysToMonths\(this\._days \+ this\._milliseconds \/ 86400000\)/g, 'monthsToDays(this._days + this._milliseconds / 86400000)'],
    ],
    inputs: fc.record({ days: fc.integer({ min: 1, max: 365 }), hours: fc.integer({ min: 0, max: 23 }) }),
    testFn: (input: unknown) => {
      const mod = mutatedMod()
      const orig = originalMoment.duration(input as Record<string, number>)
  ...
  },
  {
    ...
    inputs: fc.date({ noInvalidDate: true }),
    testFn: (input: unknown) => {
      return mutatedMoment(input).format('YYYY-MM-DD') === originalMoment(input as Date).format('YYYY-MM-DD')
    },
  },
  ...
  // === Phase 2: parse-format.ts ===
  {
    ...
    file: 'src/units.ts',
    patterns: [
      [/if \(\(y & 3\) !== 0\) \{return false;\}/g, 'if ((y & 3) !== 0) {return true;}'],
    ],
    inputs: fc.constantFrom(2023, 2024, 1900, 2000),
    testFn: (input: unknown) => {
      return mutatedMoment(new Date(input as number, 0, 1)).isLeapYear() === originalMoment(new Date(input as number, 0, 1)).isLeapYear()
    },
  },
])

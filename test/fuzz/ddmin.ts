 ...
 */
export function ddmin<T>(input: T[], test: (candidate: T[]) => boolean): T[] {
  if (input.length === 0) {
    return input;
  }
...
export function ddminString(input: string, test: (candidate: string) => boolean): string {
...
export function ddminArray<T>(input: T[], test: (candidate: T[]) => boolean): T[] {

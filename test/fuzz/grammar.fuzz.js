...
function g(buf) {
  ...
  if (useExt) {
    switch (dateType) {
      case 0:
        datePart = `${year}-${pad(month, 2)}-${pad(dayInMonth, 2)}`;
        break;
      case 1:
        datePart = `${year}-W${pad(weekNum, 2)}-${weekDay}`;
        break;
      case 2:
        datePart = `${year}-W${pad(weekNum, 2)}`;
        break;
      case 3:
        datePart = `${year}-${pad(dayOfYear, 3)}`;
        break;
      case 4:
        datePart = `${year}-${pad(month, 2)}`;
        break;
      default:
        datePart = `${year}-${pad(month, 2)}-${pad(dayInMonth, 2)}`;
        break;

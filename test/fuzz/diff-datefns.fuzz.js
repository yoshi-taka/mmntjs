...
import { format, addDays, addMonths, addYears, addHours, addMinutes, addSeconds, addWeeks } from 'date-fns'
...
export function fuzz(buf) {
  ...
  try {
    ...
    if (m2Val !== dateVal) {
      throw new Error(`valueOf mismatch for ts=${ts}: moment2=${m2Val}, date-fns=${dateVal}`)
    }
    const m2Fmt = m2.format('YYYY-MM-DD')
    const dateFmt = format(date, 'yyyy-MM-dd')
    if (m2Fmt !== dateFmt) {
      throw new Error(`format mismatch for ts=${ts}: moment2="${m2Fmt}", date-fns="${dateFmt}"`)
    }
    if (buf.length >= 8) {
      ...
      const addFn = { day: addDays, month: addMonths, year: addYears, hour: addHours, minute: addMinutes, second: addSeconds, week: addWeeks }[unit]
      if (addFn) {
        const m2Add = m2.clone().add(amount, unit)
        const dateAdd = addFn(date, amount)
        if (m2Add.valueOf() !== dateAdd.getTime()) {
          throw new Error(`add(${amount}, ${unit}) mismatch for ts=${ts}: moment2=${m2Add.valueOf()}, date-fns=${dateAdd.getTime()}`)
        }
      }
    }
  } catch (error) {

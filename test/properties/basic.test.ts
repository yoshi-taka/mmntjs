  ...
  test("format with short/ISO/12h patterns matches moment", () => {
    fc.assert(
      fc.property(safeDates, (date) => {
        ...
        expect(m2.format("YYYY-MM-DDTHH:mm:ss.SSSZ")).toBe(
          mOrig.format("YYYY-MM-DDTHH:mm:ss.SSSZ"),
        );
    ...
    fc.assert(
      fc.property(
        ...
        (input, fmt) => {
          ...
          expect(f2.unusedTokens).toEqual(fOrig.unusedTokens);
          expect(
            Math.abs((f2.charsLeftOver as number) - (fOrig.charsLeftOver as number)),
  ...
  test("format with combined tokens matches moment", () => {
    fc.assert(
      fc.property(safeDates, (date) => {
        ...
        expect(m2.format("dddd, MMMM Do YYYY, h:mm:ss A")).toBe(
          mOrig.format("dddd, MMMM Do YYYY, h:mm:ss A"),
        );
        expect(m2.format("[Year:] YYYY, [Quarter] Q, [Week] WW")).toBe(
          mOrig.format("[Year:] YYYY, [Quarter] Q, [Week] WW"),
        );
        expect(m2.format("GGGG-[W]WW")).toBe(mOrig.format("GGGG-[W]WW"));
        expect(m2.format("ddd, DD MMM YYYY HH:mm:ss ZZ")).toBe(
          mOrig.format("ddd, DD MMM YYYY HH:mm:ss ZZ"),
        );
      }),
      { numRuns: 50 },
    );

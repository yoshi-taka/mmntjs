...
import { kmLocale } from "../../src/locale/km";
import { defineLocale } from "../../src/locale";

defineLocale("km", kmLocale);
...
test("parse", function (assert) {
  ...
  for (i = 0; i < 12; i++) {
    ...
    equalTestStrict(a[1], "MMM", i);
    equalTestStrict(a[0], "MMMM", i);
    equalTestStrict(a[1].toLocaleLowerCase(), "MMM", i);
    equalTestStrict(a[1].toLocaleUpperCase(), "MMM", i);
    equalTestStrict(a[0].toLocaleLowerCase(), "MMMM", i);
    equalTestStrict(a[0].toLocaleUpperCase(), "MMMM", i);
  }
});

test("format", function (assert) {
  let a = [
      ["dddd, MMMM Do YYYY, h:mm:ss a", "អាទិត្យ, កុម្ភៈ ទី១៤ ២០១០, ៣:២៥:៥០ ល្ងាច"],
      ["ddd, hA", "អា, ៣ល្ងាច"],
...
test("format week", function (assert) {
  let expected = "អាទិត្យ អា អា_ច័ន្ទ ច ច_អង្គារ អ អ_ពុធ ព ព_ព្រហស្បតិ៍ ព្រ ព្រ_សុក្រ សុ សុ_សៅរ៍ ស ស".split("_"),
    i;
  for (i = 0; i < expected.length; i++) {
...
test("from", function (assert) {
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ h: 5 }), true), "៥ ម៉ោង", "5 hours = 5 ម៉ោង");
  assert.equal(
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ h: 36 }), true), "២ ថ្ងៃ", "36 hours = 2 ថ្ងៃ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 1 }), true), "មួយថ្ងៃ", "1 day = មួយថ្ងៃ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 5 }), true), "៥ ថ្ងៃ", "5 days = 5 ថ្ងៃ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 25 }), true), "២៥ ថ្ងៃ", "25 days = 25 ថ្ងៃ");
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 46 }), true), "២ ខែ", "46 days = 2 ខែ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 74 }), true), "២ ខែ", "75 days = 2 ខែ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 76 }), true), "៣ ខែ", "76 days = 3 ខែ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ M: 1 }), true), "មួយខែ", "1 month = មួយខែ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ M: 5 }), true), "៥ ខែ", "5 months = 5 ខែ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 345 }), true), "មួយឆ្នាំ", "345 days = មួយឆ្នាំ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 548 }), true), "២ ឆ្នាំ", "548 days = 2 ឆ្នាំ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ y: 1 }), true), "មួយឆ្នាំ", "1 year = មួយឆ្នាំ");
  assert.equal(start.from(moment([2007, 1, 28]).add({ y: 5 }), true), "៥ ឆ្នាំ", "5 years = 5 ឆ្នាំ");
});
...
test("calendar day", function (assert) {
  ...
  assert.equal(moment(a).calendar(), "ថ្ងៃនេះ ម៉ោង ១២:០០", "today at the same time");
  assert.equal(moment(a).add({ m: 25 }).calendar(), "ថ្ងៃនេះ ម៉ោង ១២:២៥", "Now plus 25 min");
  assert.equal(moment(a).add({ h: 1 }).calendar(), "ថ្ងៃនេះ ម៉ោង ១៣:០០", "Now plus 1 hour");
  assert.equal(moment(a).add({ d: 1 }).calendar(), "ស្អែក ម៉ោង ១២:០០", "tomorrow at the same time");
  assert.equal(moment(a).subtract({ h: 1 }).calendar(), "ថ្ងៃនេះ ម៉ោង ១១:០០", "Now minus 1 hour");
  assert.equal(
...
test("weeks year starting sunday formatted", function (assert) {
  ...
  assert.equal(moment([2012, 0, 2]).format("w ww wo"), "១ ០១ ទី១", "Jan  2 2012 should be week 1");
  assert.equal(moment([2012, 0, 8]).format("w ww wo"), "១ ០១ ទី១", "Jan  8 2012 should be week 1");
  assert.equal(moment([2012, 0, 9]).format("w ww wo"), "២ ០២ ទី២", "Jan  9 2012 should be week 2");
  assert.equal(moment([2012, 0, 15]).format("w ww wo"), "២ ០២ ទី២", "Jan 15 2012 should be week 2");
});

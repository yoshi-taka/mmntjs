...
import { elLocale } from "../../src/locale/el";
import { defineLocale } from "../../src/locale";

defineLocale("el", elLocale);
...
test("parse meridiem", function (assert) {
  ...
  assert.ok(
    b.isSame(moment(b.format("h:mm:ss a"), "h:mm:ss a", "el", true), "seconds"),
    `${b.format("h:mm:ss a")} should be equal to ${moment(
      b.format("h:mm:ss a"),
      "h:mm:ss a",
      "el",
      true,

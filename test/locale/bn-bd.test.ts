...
import { bn_bdLocale } from "../../src/locale/bn-bd";
import { defineLocale } from "../../src/locale";

defineLocale("bn-bd", bn_bdLocale);
...
test("format", function (assert) {
  let a = [
      ["dddd, Do MMMM YYYY, a h:mm:ss সময়", "রবিবার, ১৪ ফেব্রুয়ারি ২০১০, বিকাল ৩:২৫:৫০ সময়"],
...
test("calendar day", function (assert) {
  ...
  assert.equal(moment(a).add({ h: 3 }).calendar(), "আজ বিকাল ৩:০০ সময়", "Now plus 3 hours");
  assert.equal(
    moment(a).add({ d: 1 }).calendar(),
    "আগামীকাল দুপুর ১২:০০ সময়",
    "tomorrow at the same time",
  );
  assert.equal(moment(a).subtract({ h: 1 }).calendar(), "আজ সকাল ১১:০০ সময়", "Now minus 1 hour");
  assert.equal(
...
test("meridiem", function (assert) {
  assert.equal(moment([2011, 2, 23, 2, 30]).format("a"), "রাত", "Night");
  assert.equal(moment([2011, 2, 23, 9, 30]).format("a"), "সকাল", "morning");
  assert.equal(moment([2011, 2, 23, 14, 30]).format("a"), "দুপুর", "Noon");
  assert.equal(moment([2011, 2, 23, 17, 30]).format("a"), "বিকাল", "Afternoon");
  assert.equal(moment([2011, 2, 23, 19, 30]).format("a"), "সন্ধ্যা", "evening");

...
import { ms_myLocale } from "../../src/locale/ms-my";
import { defineLocale } from "../../src/locale";

defineLocale("ms-my", ms_myLocale);
...
test("parse", function (assert) {
  ...
  function equalTest(input: string, mmm: string, monthIdx: number) {
    assert.equal(moment(input, mmm).month(), monthIdx, `${input} sepatutnya bulan ${monthIdx + 1}`);
  }

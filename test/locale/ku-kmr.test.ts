...
import { ku_kmrLocale } from "../../src/locale/ku-kmr";
import { defineLocale } from "../../src/locale";

defineLocale("ku-kmr", ku_kmrLocale);
...
test("format", function (assert) {
  let a = [
      ["dddd, Do MMMM YYYY, h:mm:ss a", "Yekşem, 14ê Sibat 2010, 3:25:50 pn"],
      ["ddd, hA", "Yek, 3PN"],
...
test("from", function (assert) {
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ h: 22 }), true), "rojek", "22 hours = a day");
  assert.equal(start.from(moment([2007, 1, 28]).add({ h: 35 }), true), "rojek", "35 hours = a day");
  assert.equal(
    start.from(moment([2007, 1, 28]).add({ h: 36 }), true),
    "2 roj",
    "36 hours = 2 days",
  );
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 1 }), true), "rojek", "1 day = a day");
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 5 }), true), "5 roj", "5 days = 5 days");
  assert.equal(
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ M: 1 }), true), "mehek", "1 month = a month");
  assert.equal(
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ y: 1 }), true), "salek", "1 year = a year");
  assert.equal(start.from(moment([2007, 1, 28]).add({ y: 5 }), true), "5 sal", "5 years = 5 years");
});
...
test("fromNow", function (assert) {
  assert.equal(moment().add({ s: 30 }).fromNow(), "di çend sanîyeyan de", "in a few seconds");
...
test("calendar day", function (assert) {
  ...
  assert.equal(moment(a).calendar(), "Îro di saet 12:00 de", "today at the same time");
  assert.equal(moment(a).add({ m: 25 }).calendar(), "Îro di saet 12:25 de", "Now plus 25 min");
  assert.equal(moment(a).add({ h: 1 }).calendar(), "Îro di saet 13:00 de", "Now plus 1 hour");
  assert.equal(
    moment(a).add({ d: 1 }).calendar(),
    "Sibê di saet 12:00 de",
    "tomorrow at the same time",
  );
  assert.equal(moment(a).subtract({ h: 1 }).calendar(), "Îro di saet 11:00 de", "Now minus 1 hour");
  assert.equal(
...
test("calendar next week", function (assert) {
  ...
  for (i = 2; i < 7; i++) {
    ...
    assert.equal(m.calendar(), m.format("dddd [di saet] LT [de]"), `Today + ${i} days end of day`);
  }
});
...
test("weekdays strict parsing", function (assert) {
  let m = moment("2015-01-01T12", moment.ISO_8601, true),

...
import { swLocale } from "../../src/locale/sw";
import { defineLocale } from "../../src/locale";

defineLocale("sw", swLocale);
...
test("from", function (assert) {
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ d: 5 }), true), "siku 5", "5 days = 5 days");
  assert.equal(
...
test("calendar day", function (assert) {
  ...
  assert.equal(moment(a).calendar(), "leo saa 12:00 PM", "today at the same time");
  assert.equal(moment(a).add({ m: 25 }).calendar(), "leo saa 12:25 PM", "Now plus 25 min");
  assert.equal(moment(a).add({ h: 1 }).calendar(), "leo saa 01:00 PM", "Now plus 1 hour");
  assert.equal(
    moment(a).add({ d: 1 }).calendar(),
    "kesho saa 12:00 PM",
    "tomorrow at the same time",
  );
  assert.equal(moment(a).subtract({ h: 1 }).calendar(), "leo saa 11:00 AM", "Now minus 1 hour");
  assert.equal(

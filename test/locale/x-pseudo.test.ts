...
import { x_pseudoLocale } from "../../src/locale/x-pseudo";
import { defineLocale } from "../../src/locale";

defineLocale("x-pseudo", x_pseudoLocale);
...
test("calendar day", function (assert) {
  ...
  assert.equal(moment(a).calendar(), "T~ódá~ý át 12:00", "today at the same time");
  assert.equal(moment(a).add({ m: 25 }).calendar(), "T~ódá~ý át 12:25", "Now plus 25 min");
  assert.equal(moment(a).add({ h: 1 }).calendar(), "T~ódá~ý át 13:00", "Now plus 1 hour");
  assert.equal(
    moment(a).add({ d: 1 }).calendar(),
    "T~ómó~rró~w át 12:00",
    "tomorrow at the same time",
  );
  assert.equal(moment(a).subtract({ h: 1 }).calendar(), "T~ódá~ý át 11:00", "Now minus 1 hour");
  assert.equal(

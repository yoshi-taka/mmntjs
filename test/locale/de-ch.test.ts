...
import { de_chLocale } from "../../src/locale/de-ch";
import { defineLocale } from "../../src/locale";

defineLocale("de-ch", de_chLocale);

localeModule("de-ch");

test("parse", function (assert) {
  let tests =
      "Januar Jan._Februar Feb._März März_April Apr._Mai Mai_Juni Juni_Juli Juli_August Aug._September Sep._Oktober Okt._November Nov._Dezember Dez.".split(
        "_",
      ),
    i;
...
test("duration humanize week threshold", function (assert) {
  assert.equal(moment.duration(1, "week").humanize({ d: 7, w: 4 }), "eine Woche", "a week");
  assert.equal(moment.duration(-1, "week").humanize({ d: 7, w: 4 }), "eine Woche", "a week");
  assert.equal(
  ...
  assert.equal(moment.duration(2, "week").humanize({ d: 7, w: 4 }), "2 Wochen", "2 weeks");
  assert.equal(moment.duration(-2, "week").humanize({ d: 7, w: 4 }), "2 Wochen", "2 weeks");
  assert.equal(

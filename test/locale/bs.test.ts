...
test("parse", function (assert) {
  ...
  function equalTest(input: string, mmm: string, monthIdx: number) {
    assert.equal(
      ...
      monthIdx,
      `${input} should be month ${monthIdx + 1} inp ${mmm}`,
    );
  }
...
test("now from now", function (assert) {
  ...
  assert.equal(moment().subtract({ h: 1 }).fromNow(), "prije jedan sat", "1 hour ago");
});

test("fromNow", function (assert) {
  assert.equal(moment().add({ s: 30 }).fromNow(), "za par sekundi", "in a few seconds");
  assert.equal(moment().add({ m: 1 }).fromNow(), "za jednu minutu", "in 1 minute");

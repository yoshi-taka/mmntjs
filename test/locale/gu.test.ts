...
import { guLocale } from "../../src/locale/gu";
import { defineLocale } from "../../src/locale";

defineLocale("gu", guLocale);
...
test("now from now", function (assert) {
  assert.equal(moment().fromNow(), "અમુક પળો પહેલા", "now from now should display as in the past");
});
...
test("calendar day", function (assert) {
  ...
  assert.equal(moment(a).calendar(), "આજ બપોર ૧૨:૦૦ વાગ્યે", "today at the same time");
  assert.equal(moment(a).add({ m: 25 }).calendar(), "આજ બપોર ૧૨:૨૫ વાગ્યે", "Now plus 25 min");
  assert.equal(moment(a).add({ h: 3 }).calendar(), "આજ બપોર ૩:૦૦ વાગ્યે", "Now plus 3 hour");
  assert.equal(
    moment(a).add({ d: 1 }).calendar(),
    "કાલે બપોર ૧૨:૦૦ વાગ્યે",
    "tomorrow at the same time",
  );
  assert.equal(moment(a).subtract({ h: 1 }).calendar(), "આજ બપોર ૧૧:૦૦ વાગ્યે", "Now minus 1 hour");
  assert.equal(

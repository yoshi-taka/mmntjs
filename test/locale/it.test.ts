...
import { itLocale } from "../../src/locale/it";
...
test("fromNow", function (assert) {
  assert.equal(moment().add({ s: 30 }).fromNow(), "tra alcuni secondi", "tra seconds");
...
test("calendar next week", function (assert) {
  ...
  for (i = 2; i < 7; i++) {
    ...
    assert.equal(
      m.calendar(),
      m.format(`dddd [a${m.hours() > 1 ? "lle " : m.hours() === 0 ? " " : "ll'"}]LT`),
...
test("calendar last week", function (assert) {
  ...
  for (i = 2; i < 7; i++) {
    ...
    datestring =
      weekday === 0
        ? `[La scorsa] dddd [a${m.hours() > 1 ? "lle " : m.hours() === 0 ? " " : "ll'"}]LT`
        : `[Lo scorso] dddd [a${m.hours() > 1 ? "lle " : m.hours() === 0 ? " " : "ll'"}]LT`;

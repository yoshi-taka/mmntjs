...
import { tkLocale } from "../../src/locale/tk";
import { defineLocale } from "../../src/locale";

defineLocale("tk", tkLocale);
...
test("format", function (assert) {
  let a = [
      ["dddd, MMMM Do YYYY, h:mm:ss a", "Ýekşenbe, Fewral 14 2010, 3:25:50 pm"],
      ["ddd, hA", "Ýek, 3PM"],
...
test("from", function (assert) {
  ...
  assert.equal(start.from(moment([2007, 1, 28]).add({ y: 1 }), true), "bir ýyl", "1 year = a year");
  assert.equal(start.from(moment([2007, 1, 28]).add({ y: 5 }), true), "5 ýyl", "5 years = 5 years");
});
...
test("fromNow", function (assert) {
  assert.equal(moment().add({ s: 30 }).fromNow(), "birnäçe sekunt soň", "in a few seconds");
...
test("calendar day", function (assert) {
  ...
  assert.equal(moment(a).calendar(), "bugün sagat 12:00", "today at the same time");
  assert.equal(moment(a).add({ m: 25 }).calendar(), "bugün sagat 12:25", "Now plus 25 min");
  assert.equal(moment(a).add({ h: 1 }).calendar(), "bugün sagat 13:00", "Now plus 1 hour");
  assert.equal(
    moment(a).add({ d: 1 }).calendar(),
    "ertir sagat 12:00",
    "tomorrow at the same time",
  );
  assert.equal(moment(a).subtract({ h: 1 }).calendar(), "bugün sagat 11:00", "Now minus 1 hour");
  assert.equal(moment(a).subtract({ d: 1 }).calendar(), "düýn 12:00", "yesterday at the same time");
});
...
test("calendar last week", function (assert) {
  ...
  for (i = 2; i < 7; i++) {
    ...
    m.hours(23).minutes(59).seconds(59).milliseconds(999);
    assert.equal(m.calendar(), m.format("[geçen] dddd [sagat] LT"), `Today - ${i} days end of day`);
  }
});

...
test('day setter with string', function (assert) {
    var m = moment([2024, 0, 1]);
    m.day('Monday');
    assert.equal(m.day(), 1, 'Monday -> day 1');

    m.day('Sun');
    assert.equal(m.day(), 0, 'Sun -> day 0');

    m.day('Fr');
    assert.equal(m.day(), 5, 'Fr -> day 5');

    m.day('garbage');
    assert.equal(m.day(), 0, 'invalid string sets day to 0');

    m.day(undefined);
    assert.equal(m.day(), 0, 'undefined sets day to 0');

    m.day(null);
    assert.equal(m.day(), 0, 'null sets day to 0');
});

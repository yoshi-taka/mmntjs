import type { Moment as MomentFixed } from "./moment_fixed";
...
export class Moment {
  ...
  constructor(config: MomentConfig = {}) {
    ...
    this._isValid = c._isValid ?? !isNaN(this._d.getTime());
    ...
    for (const key of Object.keys(c)) {
      if (key.startsWith("_") && (c as Record<string, unknown>)[key] !== undefined) {
        (this as Record<string, unknown>)[key] = (c as Record<string, unknown>)[key];
      }
    }
  }
  ...
  private _gdt(method: DMethod): number {
    ...
    return ((this._d as unknown) as Record<string, (...args: unknown[]) => unknown>)[prefix + DMethodStr[method]]() as number;
  }

  private _sdt(method: DMethod, value: number): void {
    this._ensureOwnCopy();
    const prefix = this._isUTC ? "setUTC" : "set";
    ((this._d as unknown) as Record<string, (...args: unknown[]) => unknown>)[prefix + DMethodStr[method]](value);
  }
  ...
  clone(): this {
    ...
    for (const prop of momentProperties) {
      if (hasOwnProp(this, prop)) {
        (m as unknown as Record<string, unknown>)[prop] = (this as unknown as Record<string, unknown>)[prop];
      }
    }
    // CoW disabled
    return m as this;
  ...
  weekday(d?: number): number | this {
    if (d !== undefined) {
      ...
      const weekConfig = (this._getLocale()._config as Record<string, unknown>).week as { dow: number; doy?: number } | undefined ?? { dow: 0 };
    ...
    if (this._isUTC) {
      switch (u) {
        ...
        case "week": {
          ...
          const _weekCfg = (_locWeek._config as Record<string, unknown>).week as { dow: number; doy?: number } | undefined ?? { dow: 0 };
  ...
  format(format?: string): string {
    ...
    return formatMoment(this as unknown as MomentFixed, format);
  }
  ...
  calendar(ref?: MomentInput, opts?: object): string {
    ...
    if (typeof formatString === "string") {
      return formatMoment(this as unknown as MomentFixed, formatString);
    }

    return formatMoment(this as unknown as MomentFixed, "L");
  }
  ...
  week(w?: number): number | this {
    const weekConfig = (this._getLocale()._config as Record<string, unknown>).week as { dow: number; doy: number } | undefined ?? { dow: 0, doy: 6 };
    const dow = weekConfig.dow;
  ...
  max(other?: MomentInput): this {
    if (!this._isValid) {return this;}
    const otherM: this =
      other !== undefined ? momentFromAnything(other) as this : new Moment({ _d: new Date(NaN), _dClone: false }) as this;
    if (!otherM._isValid) {return otherM;}
    if (otherM.valueOf() > this.valueOf()) {return otherM;}
    return this;
  }

  min(other?: MomentInput): this {
    if (!this._isValid) {return this;}
    const otherM: this =
      other !== undefined ? momentFromAnything(other) as this : new Moment({ _d: new Date(NaN), _dClone: false }) as this;
    if (!otherM._isValid) {return otherM;}
  ...
  parsingFlags(): Record<string, unknown> {
    ...
    const result: Record<string, unknown> = {
      ...
      charsLeftOver: this._charsLeftOver,
      empty: this._empty,
      nullInput: this._nullInput,
      invalidMonth: this._invalidMonth,
      invalidFormat: this._invalidFormat,
      userInvalidated: this._userInvalidated,
      iso: this._iso,
      parsedDateParts: this._parsedDateParts,
      meridiem: this._meridiem,
      rfc2822: this._rfc2822,
      weekdayMismatch: this._weekdayMismatch,
      isAmPm: this._bigHour,
      isParseZone: this._isParseZone,
      bigHour: this._bigHour,
    };

...
export function createFromArrayInput(
  ...
  if (!parsed) {return new Moment({ _dClone: false, _d: new Date(NaN), _i: arr, _isValid: false });}
  const overflow = checkOverflow(parsed);
  const d = createDateSafe(parsed.year, parsed.month, parsed.day, parsed.hour, parsed.minute, parsed.second, parsed.millisecond, isUTC);
  if (overflow >= 0) {return new Moment({ _dClone: false, _d: d, _i: arr, _isValid: false, _overflow: overflow });}
...
export function createFromObjectInput(
  ...
  const month = parsed.month !== undefined ? parsed.month as number : parsed.year !== undefined ? 0 : now.getMonth();
  const day = parsed.day !== undefined ? parsed.day as number : parsed.year !== undefined || parsed.month !== undefined ? 1 : now.getDate();
  ...
  if (overflow >= 0) {return new Moment({ _dClone: false, _d: d, _i: obj, _isValid: false, _overflow: overflow });}

...
function scoreParsedResult(parsed: ParsedDataLike): number {
  ...
  if (parsed.millisecond !== undefined) {
    score += 1;
  }
  return score;
}
...
export function createFromFormattedStringInput(args: FormatArgs): Moment {
  ...
  if (Array.isArray(fmt)) {
    ...
    for (const singleFmt of fmt) {
      if (singleFmt === "ISO_8601") {
        ...
        if (parsed && hasAnyValue(parsed)) {
          ...
          if (parsed._empty === true) {
            score -= 50;
          }
          if (checkOverflow(parsed) >= 0) {
            score -= 100;
          }
          if (score > bestScore) {
            bestParsed = parsed;
      ...
      if ((parsed._unusedTokens?.length ?? 0) > 0) {
        score -= 10 * (parsed._unusedTokens?.length ?? 0);
      }
      if ((parsed._charsLeftOver ?? 0) > 0) {
        score -= (parsed._charsLeftOver ?? 0) * 3;
      }
      if ((parsed._unusedInput?.length ?? 0) > 0) {
    ...
    if (bestParsed && hasAnyValue(bestParsed)) {
      ...
      if (overflow < 0) {
        if (
          strict &&
          ((bestParsed._unusedTokens?.length ?? 0) > 0 || (bestParsed._charsLeftOver ?? 0) > 0)
        ) {
      ...
      return createOverflowParsedMoment({
        str,
        format: bestFormat,
        locale,
        strict,
  ...
  if (fmt === "ISO_8601" || fmt === "RFC_2822") {
    const parsed = deps.parseString(
      str,
      undefined,
      getLocale(locale ?? getCurrentLocale()) as unknown as ParseLocale,
    );
